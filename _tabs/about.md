---
# the default layout is 'page'
icon: fas fa-info-circle
layout: page
title: About
permalink: /about/
---


Hi there! I'm a **Full Stack Developer** and a passionate **Security Researcher**.  

## As a Full Stack Developer
I specialize in building dynamic, scalable, and responsive web applications. My skill set includes:  
- Frontend frameworks like React.js.  
- Backend development with technologies like Node.js and Django.  
- Expertise in integrating APIs and making applications interactive.  

I love creating projects that solve real-world problems and elevate user experiences.  

## As a Security Researcher
I'm dedicated to making the web a safer place by identifying vulnerabilities and exploring web security concepts.  
- Experienced in **Capture the Flag (CTF)** challenges.  
- Proficient in conducting **Vulnerability Assessments** and penetration testing.  
- Skilled in writing detailed bug reports and working with security teams to resolve issues.  

## Beyond Work
When I'm not coding or researching, I enjoy contributing to the tech community, learning about emerging trends in cybersecurity, and collaborating on exciting projects.  

Feel free to reach out—let's build something incredible together!


## Connect with Me

- [Link to your LinkedIn]
- [Link to your Twitter]
- [Link to your GitHub]
- [Link to any other social media or personal website]
